	...
	return -1;
	 ...
	 //logp("before x\n");
         if(file_is_included(conf->incexcdir, conf->iecount, link)) {
            rtn_stat = find_files(ff_pkt, conf, cntr, link, our_device, false);
            if (ff_pkt->linked) {
               ff_pkt->linked->FileIndex = ff_pkt->FileIndex;
            }
         }
		...
		for(ex=0; ex<conf->iecount; ex++) {
		  if(conf->incexcdir[ex]->flag
			...
			int ey;
			if((rtn_stat=find_files(ff_pkt, conf, cntr,
	 ...
	 if(rtn_stat) break;
       ...
       */
      if(!rtn_stat) send_file(dir_ff_pkt, top_level, conf, cntr);       /* handle directory entry */
      if (ff_pkt->linked) {
