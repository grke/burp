...
struct manios *manios_open_phase2(struct sdirs *sdirs, enum protocol p)
{
	struct manios *m=NULL;
	if(!(m=manios_alloc())
	  || !(m->current=manio_open(sdirs->cmanifest, "rb", p))
	  || !(m->phase1=manio_open_phase1(sdirs->phase1data, "rb", p))
	  || !(m->changed=manio_open_phase2(sdirs->changed, "wb", p))
	  || !(m->unchanged=manio_open_phase2(sdirs->unchanged, "wb", p)))
		manios_close(&m);
	return m;
}
