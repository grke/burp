...
static int do_backup_server(const char *basedir, const char *current, const char *working, const char *currentdata, const char *finishing, struct config *cconf, const char *manifest, const char *phase1data, const char *phase2data, const char *unchangeddata, const char *client, const char *cversion, struct cntr *p1cntr, struct cntr *cntr, int resume, const char *incexc)
{
	...
	if(!(timestamp=prepend_s(working, "timestamp"))
	  || !(newpath=prepend_s(working, "patched.tmp"))
	  || !(cmanifest=prepend_s(current, "manifest.gz"))
	  || !(cincexc=prepend_s(current, "incexc"))
	  || !(datadirtmp=prepend_s(working, "data.tmp")))
	{
		log_and_send_oom(__FUNCTION__);
		goto error;
	}
	...
	if(resume)
	{
		...
		if((len=readlink(working, real, sizeof(real)-1))<0)
			len=0;
		real[len]='\0';
		if(!(realworking=prepend_s(basedir, real)))
		{
			log_and_send_oom(__FUNCTION__);
			goto error;
		}
	...
	else
	{
		...
		if(get_new_timestamp(cconf, basedir, tstmp, sizeof(tstmp)))
			goto error;
		if(!(realworking=prepend_s(basedir, tstmp)))
		{
			log_and_send_oom(__FUNCTION__);
			goto error;
		}
...
static int check_for_rubble(const char *basedir, const char *current, const char *working, const char *currentdata, const char *finishing, struct config *cconf, const char *phase1data, const char *phase2data, const char *unchangeddata, const char *manifest, const char *client, struct cntr *p1cntr, struct cntr *cntr, int *resume, const char *incexc)
{
	...
	realwork[len]='\0';
	if(!(fullrealwork=prepend_s(basedir, realwork)))
	{
		ret=-1;
		goto end;
	}
...
static int child(struct config *conf, struct config *cconf, const char *client, const char *cversion, const char *incexc, int srestore, char cmd, char *buf, char **gotlock, int *timer_ret, struct cntr *p1cntr, struct cntr *cntr)
{
	...
	if(!(basedir=prepend_s(cconf->directory, client))
	  || !(working=prepend_s(basedir, "working"))
	  || !(finishing=prepend_s(basedir, "finishing"))
	  || !(current=prepend_s(basedir, "current"))
	  || !(currentdata=prepend_s(current, "data"))
	  || !(manifest=prepend_s(working, "manifest.gz"))
	  || !(datadirtmp=prepend_s(working, "data.tmp"))
	  || !(phase1data=prepend_s(working, "phase1.gz"))
	  || !(phase2data=prepend_s(working, "phase2"))
	  || !(unchangeddata=prepend_s(working, "unchanged"))
	  || !(lockbasedir=prepend_s(client_lockdir, client))
	  || !(lockfile=prepend_s(lockbasedir, "lockfile")))
	{
		log_and_send_oom(__FUNCTION__);
		ret=-1;
	}
