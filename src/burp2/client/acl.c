...
static int get_acl_string(acl_t acl, char **acltext, size_t *alen, const char *path, char type, struct conf *conf)
{
	char pre[10]="";
	char *tmp=NULL;
	ssize_t tlen=0;
	...
	{
		logw(conf, "could not get ACL text of '%s'\n", path);
		return 0; // carry on
	}
...
int get_acl(const char *path, struct stat *statp, char **acltext, size_t *alen, struct conf *conf)
{
	acl_t acl=NULL;
...
static int do_set_acl(const char *path, struct stat *statp, const char *acltext, size_t alen, int acltype, struct conf *conf)
{
	acl_t acl;
	if(!(acl=acl_from_text(acltext)))
	{
		...
		logw(conf, "acl_from_text error on %s (%s): %s\n",
			path, acltext, strerror(errno));
		return -1;
	}
//#ifndef HAVE_FREEBSD_OS // Bacula says that acl_valid fails on valid input
	...
	if(acl_valid(acl))
	{
		...
		logw(conf, "acl_valid error on %s: %s", path, strerror(errno));
		acl_free(acl);
		return -1;
	}
//#endif
	if(acl_set_file(path, acltype, acl))
	{
		...
		logw(conf, "acl set error on %s: %s", path, strerror(errno));
		acl_free(acl);
		return -1;
	}
	acl_free(acl);
...
int set_acl(const char *path, struct stat *statp, const char *acltext, size_t alen, char cmd, struct conf *conf)
{
	switch(cmd)
	{
