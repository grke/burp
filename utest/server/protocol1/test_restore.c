...
#include "../../../src/iobuf.h"
#include "../../../src/sbuf.h"
#include "../../../src/server/protocol1/restore.h"
#include "../../builders/build_asfd_mock.h"
#include "../../builders/build_file.h"

#define BASE	"utest_server_protocol1_restore"
...
START_TEST(test_protocol1_verify_file_non_existent)
{
	struct asfd *asfd;
	struct cntr *cntr;
	struct sbuf *sb;
...
START_TEST(test_protocol1_verify_file_no_md5sum)
{
	do_md5sum_test("0", 1/*warnings*/, setup_no_md5sum);
}
END_TEST

START_TEST(test_protocol1_verify_file_md5sum_no_match)
{
	do_md5sum_test("0:0", 1 /*warnings*/, setup_md5sum_no_match);
}
END_TEST

START_TEST(test_protocol1_verify_file_md5sum_match)
{
...
Suite *suite_server_protocol1_restore(void)
{
	...
	s=suite_create("server_protocol1_restore");

	tc_core=tcase_create("Core");
	tcase_add_test(tc_core, test_protocol1_verify_file_non_existent);
	tcase_add_test(tc_core, test_protocol1_verify_file_no_md5sum);
	tcase_add_test(tc_core, test_protocol1_verify_file_md5sum_no_match);
	tcase_add_test(tc_core, test_protocol1_verify_file_md5sum_match);
	tcase_add_test(tc_core, test_protocol1_verify_file_gzip_read_failure);
	tcase_add_test(tc_core, test_protocol1_restore_file_not_found);
